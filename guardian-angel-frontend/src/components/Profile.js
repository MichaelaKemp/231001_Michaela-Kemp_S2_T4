...
const fetchRequests = async () => {
  ...
  const fetchLikes = async () => {
    try {
      const likesRes = await axios.get(`http://localhost:5000/user/${userId}/likes`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const fetchComments = async () => {
    try {
      const commentsRes = await axios.get(`http://localhost:5000/user/${userId}/comments`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleUserResponse = async (requestId, userId, action) => {
    try {
      await axios.post(`http://localhost:5000/requests/${requestId}/respond`, 
        { userId, action }, 
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
  ...
  const handleCancelRequest = async (requestId) => {
    try {
      await axios.post(`http://localhost:5000/requests/${requestId}/cancel`, {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleUpdateRequest = async (updatedRequest) => {
    ...
    try {
      await axios.put(`http://localhost:5000/requests/${updatedRequest.request_id}`, minimalRequestData, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
                      ...
                      <ul>
                        {request.acceptedUsers.map(user => (
                          <li key={user.id} className="accepted-user">
                            <img src={`http://localhost:5000/uploads/${user.profile_image}`} alt={`${user.name}'s profile`} className="accepted-user-image" />
