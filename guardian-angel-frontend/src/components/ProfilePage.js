...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        const profileResponse = await axios.get(`http://localhost:5000/user/${userId}`, {
    ...
    const fetchLikes = async () => {
      try {
        const likesResponse = await axios.get(`http://localhost:5000/user/${userId}/likes`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
    ...
    const fetchComments = async () => {
      try {
        const commentsResponse = await axios.get(`http://localhost:5000/user/${userId}/comments`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
    ...
    const fetchAnalytics = async () => {
      try {
        const analyticsResponse = await axios.get(`http://localhost:5000/analytics/${userId}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
  ...
  const handleComment = async () => {
    ...
    try {
      await axios.post(`http://localhost:5000/user/${userId}/comment`, { comment: newComment }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
